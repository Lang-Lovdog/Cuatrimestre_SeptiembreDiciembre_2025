...
def select_fields_for_comparison(df, comparison_type="all"):
    """
    Select which fields to compare
    
    Parameters:
    - df: DataFrame
    - comparison_type: "all", "target", "custom", "numeric_only", "categorical_only"
    """
    columns = df.columns.tolist()
    
    if comparison_type == "all":
        return columns, "all_fields"
        ...
        while True:
            try:
                choice = int(input("Enter the number of your target variable: "))
                if 1 <= choice <= len(columns):
                    target = columns[choice - 1]
                    # Compare target with all other fields
                    other_fields = [col for col in columns if col != target]
                    return [target], other_fields, f"target_{target}"
                else:
        ...
        print("\nSelect fields to compare (enter numbers separated by spaces):")
        selected_indices = input("Fields: ").strip().split()
        
        try:
            selected_fields = [columns[int(idx)-1] for idx in selected_indices]
            print(f"✅ Selected fields: {selected_fields}")
            return selected_fields, selected_fields, "custom_fields"
        except (ValueError, IndexError):
            print("❌ Invalid selection. Using all fields.")
            return columns, "all_fields"
    
    elif comparison_type == "numeric_only":
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        print(f"🔢 Selected {len(numeric_cols)} numeric fields")
        return numeric_cols, "numeric_only"
    
    elif comparison_type == "categorical_only":
        categorical_cols = df.select_dtypes(include=['object']).columns.tolist()
        print(f"📋 Selected {len(categorical_cols)} categorical fields")
        return categorical_cols, "categorical_only"
    
    else:
        return columns, "all_fields"

def create_single_page_plots(df, categorical_info, output_pdf_path, theme='viridis', 
                           use_tex=False, fields_x=None, fields_y=None, comparison_name=""):
    ...
    if fields_x is None or fields_y is None:
        # Compare all fields with all fields
        columns = df_processed.columns
        pairs = list(permutations(columns, 2))
        comparison_desc = "all_fields"
    else:
        ...
        for x in fields_x:
            for y in fields_y:
                if x != y:  # Avoid self-comparison
                    pairs.append((x, y))
        comparison_desc = comparison_name
    
    print(f"   🎨 Generating {len(pairs)} permutation plots ({comparison_desc})...")
...
def main(csv_file_path, 
         ...
         comparison_type="all",
         custom_fields_x=None,
         custom_fields_y=None):
    """
    ...
    - comparison_type: "all", "target", "custom", "numeric_only", "categorical_only"
    - custom_fields_x: List of fields for X-axis (for custom comparison)
    - custom_fields_y: List of fields for Y-axis (for custom comparison)
    """
    ...
    # Load data first to get columns for field selection
    df = load_and_preprocess_data(csv_file_path)
    
    # Select fields for comparison
    if custom_fields_x is not None and custom_fields_y is not None:
        fields_x = custom_fields_x
        fields_y = custom_fields_y
        comparison_name = "custom_selection"
        print(f"🔧 Using custom field selection: {len(fields_x)}×{len(fields_y)} comparisons")
    else:
        fields_x, fields_y, comparison_name = select_fields_for_comparison(df, comparison_type)
    
    # Generate default output names if not provided
    base_name = os.path.splitext(os.path.basename(csv_file_path))[0]
    if pdf_output_name is None:
        pdf_output_name = f"{base_name}_{comparison_name}_analysis_{theme}.pdf"
    if extra_analysis_dir == "analysis_output":
        extra_analysis_dir = f"{base_name}_{comparison_name}_analysis_{theme}"
    ...
    pairs = create_single_page_plots(df, categorical_info, pdf_output_name, theme, use_tex, 
                                   fields_x, fields_y, comparison_name)
    ...
    print(f"🔍 Comparison Type: {comparison_name}")
    print("=" * 70)

# Example usage and theme testing
def run_comprehensive_analysis(csv_path, themes=None, comparison_type="all"):
    ...
    for theme in themes:
        ...
        try:
            main(csv_path,
                 pdf_output_name=f"House_Price_Analysis_{comparison_type}_{theme.capitalize()}.pdf",
                 analyze_categorical=True,
                 export_extra_analysis=True,
                 extra_analysis_dir=f"House_Price_Analysis_{comparison_type}_{theme}",
                 theme=theme,
                 use_tex=False,
                 export_tex=False,
...
def quick_analysis(csv_path, theme='viridis', comparison_type="all"):
    """Run a quick analysis with default settings"""
    print("⚡ RUNNING QUICK ANALYSIS...")
    main(csv_path,
         pdf_output_name=f"House_Price_Quick_Analysis_{comparison_type}.pdf",
         analyze_categorical=True,
...
if __name__ == "__main__":
    # Your dataset path
    csv_file = "./House_PriceReg-Processed.csv"
    ...
    print("1. All fields vs all fields (default)")
    print("2. Target variable vs all other fields")
    print("3. Custom field selection")
    print("4. Numeric fields only")
    print("5. Categorical fields only")
    
    comp_choice = input("Enter comparison type (1-5): ").strip() or "1"
    ...
    comparison_type = comparison_map.get(comp_choice, "all")
    
    if choice == "1":
        quick_analysis(csv_file, comparison_type=comparison_type)
        ...
        try:
            ...
            main(csv_file, theme=selected_theme, comparison_type=comparison_type)
        except (ValueError, IndexError):
            print("Invalid choice, using default 'viridis' theme")
            main(csv_file, theme='viridis', comparison_type=comparison_type)
            
    elif choice == "3":
        run_comprehensive_analysis(csv_file, comparison_type=comparison_type)
